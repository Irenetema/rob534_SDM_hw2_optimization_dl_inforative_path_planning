...
class Game:
    ...
    def tick(self):
        ...
        if self.robot.getLoc() == self._goal:
            ...
            if self.robot.getLoc() in self._wrong_goals:
                self.score -= 400
                # *******
                self.navigator.better_goal_loc = self._goal
            else:
                self.score -= 1
            return False
    ...
    def _updateMap(self, robot, exploredMap, truthMap):
        ...
        if robot.getLoc()[0] > 27 or robot.getLoc()[0] < 0 or robot.getLoc()[1] < 0 or robot.getLoc()[1] > 27:
            raise ValueError(
                f"Robot has left the map. It is at position: {robot.getLoc()}, outside of the (0-27, 0-27) map boundary")
    ...
    def plot_path(self, title_name=''):
        """
        Plot the path taken by the robot on top of the map
        Returns:
        """
        ...
        colors = ['b', 'r']
        # plt.imshow(self.truthMap, cmap=plt.get_cmap('bone'))
        ax1.imshow(self.exploredMap, cmap=plt.get_cmap('bone'))
        if title_name is not None:
            # fig.suptitle(title_name, fontSize=20)
            ax1.set_title(title_name)

        # cast path to numpy array so indexing is nicer
        path = np.array(self.navigator.path)
        print('path shape:', path.shape)
        ax1.plot(path[:, 0], path[:, 1], color=colors[1], linewidth=4)

        ax2.imshow(self.truthMap, cmap=plt.get_cmap('bone'))
